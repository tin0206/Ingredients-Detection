...
# ---------- LOAD YAML ----------
def load_class_map():
    with open(DATA5_YAML, "r", encoding="utf-8") as f:
...
def main():
    ...
    for split, target in SPLITS.items():
        ...
        if existing:
            img_idx = max(int(p.stem) for p in existing) + 1
        else:
            img_idx = 0
        ...
        while img_idx < target:
            ...
            available_classes = [k for k, v in pool.items() if len(v) > 0]
